...
class constants:
    ...
    consumption_rate = 1.0  #consumption rate, this has to be 1 as all the rest will be normalized to these units.
    ...
    L_threshold = 1.0 #threshold below which the consumer does not consume more resources from that cell, in consumption rate units. 
...
def resorurces_evol(cnt, t, L, max_land, l):
    ...
    for e in t:
        ...
        L = L + p
        s = np.full( len(l), 0.0) #be mindfull, it has to be float!!!! 0.0
        ...
        mask = (L[l]  - cnt.consumption_rate <= cnt.L_threshold)  &  (L[l] - cnt.L_threshold + cnt.sea_productivity >= cnt.consumption_rate ) 
        ind_seac = np.where(mask)[0]
        
        land_margin = np.array(L[l[ind_seac]]) - cnt.L_threshold


        L[l[ind_seac]] = cnt.L_threshold

        #for j, lm in zip(ind_seac, land_margin):
        #    s[j] = cnt.consumption_rate - lm

            #if e == 1:
             #   print ('ssss', s[j], ind_seac, j, lm, cnt.consumption_rate - lm )
        s[ind_seac] = cnt.consumption_rate - np.array(land_margin)
...
def acumulated_sea_consumption(cnt, sea_resources):    
    ...
    this is the total sea condumtion by all the agents through all the time, after a burnout 
    period is remobed'''
